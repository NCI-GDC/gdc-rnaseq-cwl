...
steps:
  process_bam_files:
    run: ./input_bam_processing_workflow.cwl
  ...
  run_merge_fastq_array_workflow:
    run: ./merge_fastq_arrays_workflow.cwl
    in:
      bam_pe_fastqs:
  ...
  run_merge_genome_bams:
    run: ./merge_and_index_workflow.cwl
    in:
      input_bam:
  ...
  run_collect_metrics:
    run: ./rnaseq_metrics_workflow.cwl
    in:
      threads: threads
      ref_flat: ref_flat
